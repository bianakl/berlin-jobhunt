...
import { Star, ExternalLink, Pencil, Trash2, Plus, Building2, Search, RefreshCw, X, CheckSquare, Loader2, Link2, Mail, Users, Kanban, Sparkles } from 'lucide-react';
...
export default function Companies({ companies, jobs, onAddCompany, onEditCompany, onDeleteCompany, onAddJob, onUpdateCompany }) {
  ...
  const [sort, setSort] = useState('default'); // 'default' | 'az' | 'most'
  ...
  const filtered = companies
    ...
    .sort((a, b) => {
      ...
      // default: has PM positions first, then alpha
        ...
        <div className="flex items-center gap-2">
          {checkableCompanies.length > 0 && (
            <button
              ...
              style={{ background: checkingAll ? '#f3f4f6' : 'rgba(99,102,241,0.06)', color: checkingAll ? '#9ca3af' : '#6366f1', border: '1px solid rgba(99,102,241,0.15)' }}
            >
...
function CompanyCard({ company, companyJobs, onEdit, onDelete, onAddJob, onUpdateCompany }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const [checking, setChecking] = useState(false);
  const [addingManual, setAddingManual] = useState(false);
  const [manualTitle, setManualTitle] = useState('');
  const [manualUrl, setManualUrl] = useState('');
  const [analyses, setAnalyses] = useState({}); // { [posId]: { loading, score, summary, strengths, gaps, error } }
  ...
  return (
    <div
      ...
      style={{ background: '#fff', borderColor: positions.length > 0 ? 'rgba(99,102,241,0.25)' : '#e8e8f4' }}
    ...
    >
      ...
      {/* Outreach tracking row */}
      {(company.linkedinUrl || company.connections || company.referral || company.viaForm || company.email) && (
        <div className="flex flex-wrap items-center gap-1.5">
          {company.linkedinUrl && (
            <a
              href={company.linkedinUrl} target="_blank" rel="noreferrer"
              className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded font-medium transition-all"
          ...
          {company.referral && (
            <span className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ background: 'rgba(34,197,94,0.08)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.2)' }}>
              ✓ Referral
            </span>
          )}
          {company.viaForm && (
            <span className="text-[10px] px-1.5 py-0.5 rounded font-medium" style={{ background: 'rgba(99,102,241,0.08)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.2)' }}>
              ✓ Via form
            </span>
          )}
        </div>
      ...
      {positions.length > 0 && (
        <div className="flex flex-col gap-2.5">
          {positions.map((pos) => {
            ...
            return (
              <div key={pos.id} className="rounded-lg p-2.5 group/pos" style={{ background: '#f9fafb', border: '1px solid #f0f0f8' }}>
                {/* Title row */}
                <div className="flex items-start gap-1.5 mb-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {pos.team && pos.location && pos.snippet && (
                      <p className="text-[10px] mt-0.5 leading-relaxed line-clamp-2" style={{ color: '#b0b8c8' }}>
                        {pos.snippet}
                      </p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      title="Analyze compatibility with your CV"
                    >
      ...
      {/* Add manually */}
      {addingManual ? (
        <div className="flex flex-col gap-1.5 p-2.5 rounded-lg" style={{ background: '#f9fafb', border: '1px solid #e8e8f4' }}>
          <input
            autoFocus
          ...
          <div className="flex gap-1.5">
            ...
            <button
              onClick={() => { setAddingManual(false); setManualTitle(''); setManualUrl(''); }}
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs transition-all self-start"
          style={{ color: '#d1d5db' }}
          onMouseEnter={(e) => (e.currentTarget.style.color = '#6366f1')}
          onMouseLeave={(e) => (e.currentTarget.style.color = '#d1d5db')}
        >
